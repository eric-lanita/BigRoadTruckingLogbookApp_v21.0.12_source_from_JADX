...
public class LoginButton extends FacebookButtonBase {
    ...
    protected class LoginClickListener implements OnClickListener {
        ...
        protected void performLogout(Context context) {
            ...
            if (LoginButton.this.confirmLogout) {
                CharSequence string;
                CharSequence string2 = LoginButton.this.getResources().getString(C3002R.string.com_facebook_loginview_log_out_action);
                CharSequence string3 = LoginButton.this.getResources().getString(C3002R.string.com_facebook_loginview_cancel_action);
                Profile currentProfile = Profile.getCurrentProfile();
                if (currentProfile == null || currentProfile.getName() == null) {
                    string = LoginButton.this.getResources().getString(C3002R.string.com_facebook_loginview_logged_in_using_facebook);
                } else {
                    string = String.format(LoginButton.this.getResources().getString(C3002R.string.com_facebook_loginview_logged_in_as), new Object[]{currentProfile.getName()});
                }
    ...
    class C30902 extends AccessTokenTracker {
        C30902() {
        }
    ...
    private void checkToolTipSettings() {
        switch (this.toolTipMode) {
            case AUTOMATIC:
                ...
                return;
            case DISPLAY_ALWAYS:
                displayToolTip(getResources().getString(C3002R.string.com_facebook_tooltip_default));
                return;
            default:
                return;
        }
    ...
    protected void configureButton(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        if (isInEditMode()) {
            setBackgroundColor(getResources().getColor(C3002R.color.com_facebook_blue));
            this.loginText = "Log in with Facebook";
        } else {
            this.accessTokenTracker = new C30902();
        }
        setButtonText();
    }
    ...
    protected int getDefaultStyleResource() {
        return C3002R.style.com_facebook_loginview_default_style;
    }

    private void parseLoginButtonAttributes(Context context, AttributeSet attributeSet, int i, int i2) {
        this.toolTipMode = ToolTipMode.DEFAULT;
        TypedArray obtainStyledAttributes = context.getTheme().obtainStyledAttributes(attributeSet, C3002R.styleable.com_facebook_login_view, i, i2);
        try {
            this.confirmLogout = obtainStyledAttributes.getBoolean(C3002R.styleable.com_facebook_login_view_com_facebook_confirm_logout, true);
            this.loginText = obtainStyledAttributes.getString(C3002R.styleable.com_facebook_login_view_com_facebook_login_text);
            this.logoutText = obtainStyledAttributes.getString(C3002R.styleable.com_facebook_login_view_com_facebook_logout_text);
            this.toolTipMode = ToolTipMode.fromInt(obtainStyledAttributes.getInt(C3002R.styleable.com_facebook_login_view_com_facebook_tooltip_mode, ToolTipMode.DEFAULT.getValue()));
        } finally {
    ...
    protected void onMeasure(int i, int i2) {
        ...
        if (str == null) {
            str = resources.getString(C3002R.string.com_facebook_loginview_log_in_button_long);
            measureButtonWidth = measureButtonWidth(str);
            if (resolveSize(measureButtonWidth, i) < measureButtonWidth) {
                str = resources.getString(C3002R.string.com_facebook_loginview_log_in_button);
            }
        }
        measureButtonWidth = measureButtonWidth(str);
        str = this.logoutText;
        if (str == null) {
            str = resources.getString(C3002R.string.com_facebook_loginview_log_out_button);
        }
    ...
    private void setButtonText() {
        ...
        if (!isInEditMode() && AccessToken.getCurrentAccessToken() != null) {
            if (this.logoutText != null) {
                ...
                charSequence = resources.getString(C3002R.string.com_facebook_loginview_log_out_button);
            }
            setText(charSequence);
        } else if (this.loginText != null) {
            setText(this.loginText);
        } else {
            charSequence = resources.getString(C3002R.string.com_facebook_loginview_log_in_button_long);
            int width = getWidth();
            if (width != 0 && measureButtonWidth(charSequence) > width) {
                charSequence = resources.getString(C3002R.string.com_facebook_loginview_log_in_button);
            }
            setText(charSequence);
        }
    }
