...
public class ActivityLifecycleTracker {
    ...
    static class C30181 implements ActivityLifecycleCallbacks {
        C30181() {
        }
    ...
    public static void startTracking(Application application, String str) {
        if (tracking.compareAndSet(false, true)) {
            appId = str;
            application.registerActivityLifecycleCallbacks(new C30181());
        }
    }
    ...
    private static void onActivityPaused(Activity activity) {
        ...
        singleThreadExecutor.execute(new Runnable() {
            ...
            public void run() {
                ...
                ActivityLifecycleTracker.currentSession.setSessionLastEventTime(Long.valueOf(currentTimeMillis));
                if (ActivityLifecycleTracker.foregroundActivityCount.get() <= 0) {
                    ActivityLifecycleTracker.currentFuture = ActivityLifecycleTracker.singleThreadExecutor.schedule(new C30211(), (long) ActivityLifecycleTracker.getSessionTimeoutInSeconds(), TimeUnit.SECONDS);
                }
