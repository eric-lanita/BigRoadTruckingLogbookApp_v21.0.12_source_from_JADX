...
public final class GooglePlayServicesUtil extends zze {
    ...
    private static class zza extends Handler {
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                case 1:
                    int isGooglePlayServicesAvailable = GooglePlayServicesUtil.isGooglePlayServicesAvailable(this.f10489a);
                    if (GooglePlayServicesUtil.isUserRecoverableError(isGooglePlayServicesAvailable)) {
                        GooglePlayServicesUtil.zza(isGooglePlayServicesAvailable, this.f10489a);
                        return;
                    }
                    return;
                default:
    ...
    public static boolean showErrorDialogFragment(int i, Activity activity, Fragment fragment, int i2, OnCancelListener onCancelListener) {
        ...
        Dialog a = instance.m16776a((Context) activity, i, zzi.zza(fragment, GoogleApiAvailability.getInstance().zza((Context) activity, i, "d"), i2), onCancelListener);
        if (a == null) {
            return false;
        }
        instance.m16777a(activity, a, GMS_ERROR_DIALOG, onCancelListener);
        return true;
    }
    ...
    private static void zza(int i, Context context, String str, PendingIntent pendingIntent) {
        ...
        if (zzg == null) {
            zzg = resources.getString(C3176R.string.common_google_play_services_notification_ticker);
        }
        CharSequence zzd = zzh.zzd(context, i, zzbv);
        if (com.google.android.gms.common.util.zzi.zzck(context)) {
            zzab.zzbn(zzs.zzavr());
            build = new Builder(context).setSmallIcon(C3176R.drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new BigTextStyle().bigText(new StringBuilder((String.valueOf(zzg).length() + 1) + String.valueOf(zzd).length()).append(zzg).append(" ").append(zzd).toString())).addAction(C3176R.drawable.common_full_open_on_phone, resources.getString(C3176R.string.common_open_on_phone), pendingIntent).build();
        } else {
            CharSequence string = resources.getString(C3176R.string.common_google_play_services_notification_ticker);
            if (zzs.zzavn()) {
                Notification build2;
                ...
                build = new C0138d(context).m626a(17301642).m649d(string).m628a(System.currentTimeMillis()).m646c(true).m630a(pendingIntent).m636a(zzg).m642b(zzd).m639b();
            }
        }
        if (zze.zzfe(i)) {
            rv.set(false);
